...
# To disable: appjail oci run -e GODEBUG=fips140=off ...
...
if [ "$(id -u)" = "0" ]; then
    ...
    if [ "$DATA_UID" != "$SEAWEED_UID" ] || [ "$DATA_GID" != "$SEAWEED_GID" ]; then
        info "Fixing /data ownership for seaweed user (uid=$SEAWEED_UID, gid=$SEAWEED_GID)"
        if ! chown -R noroot:noroot /data; then
            warn "Failed to change ownership of /data. This may cause permission errors." >&2
            warn "If /data is read-only or has mount issues, the application may fail to start." >&2
        fi
    fi

    # Use su-exec to drop privileges and run as seaweed user
    exec su-exec noroot "$0" "$@"
fi
...
case "$1" in
    ...
    'master')
        ...
        shift
        exec weed -logtostderr=true master $ARGS $@
        ;;
    ...
    'volume')
        ...
        shift
        exec weed -logtostderr=true volume $ARGS $@
        ;;
    ...
    'volume-rust')
        ...
        shift
        if [ ! -s weed-volume ]; then
            err "Rust volume server is not available on this platform ($(uname -m))." >&2
            err "Use 'volume' for the Go volume server instead." >&2
            exit 1
        fi
    ...
    'server')
        ...
        shift
        exec weed -logtostderr=true server $ARGS $@
        ;;
